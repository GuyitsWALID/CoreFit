CREATE OR REPLACE FUNCTION public.record_offline_renewal(
  ...
  p_periods_paid integer DEFAULT 1,
...
BEGIN
  ...
  v_periods_paid := COALESCE(p_periods_paid, 1);
  IF v_periods_paid < 1 OR v_periods_paid > 120 THEN
    RAISE EXCEPTION 'Periods paid must be between 1 and 120.';
  END IF;
  ...
  v_new_expiry := CASE lower(COALESCE(v_package.duration_unit::text, 'days'))
    WHEN 'day' THEN v_start_date + make_interval(days => (v_package.duration_value::int * v_periods_paid))
    WHEN 'days' THEN v_start_date + make_interval(days => (v_package.duration_value::int * v_periods_paid))
    WHEN 'week' THEN v_start_date + make_interval(weeks => (v_package.duration_value::int * v_periods_paid))
    WHEN 'weeks' THEN v_start_date + make_interval(weeks => (v_package.duration_value::int * v_periods_paid))
    WHEN 'month' THEN v_start_date + make_interval(months => (v_package.duration_value::int * v_periods_paid))
    WHEN 'months' THEN v_start_date + make_interval(months => (v_package.duration_value::int * v_periods_paid))
    WHEN 'year' THEN v_start_date + make_interval(years => (v_package.duration_value::int * v_periods_paid))
    WHEN 'years' THEN v_start_date + make_interval(years => (v_package.duration_value::int * v_periods_paid))
    ELSE v_start_date + make_interval(days => (v_package.duration_value::int * v_periods_paid))
  END;
...
GRANT EXECUTE ON FUNCTION public.record_offline_renewal(uuid, uuid, timestamptz, integer, numeric, text, text) TO authenticated;
